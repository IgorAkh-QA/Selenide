...
public class Snippets {
    ...
    void browser_command_examples() {
        ...
        Selenide.back(); // Браузерная кнопка "НАЗАД". Используется при тестировании Wizzards (кода открытие счета в банке проходит в несколько шагов, например)
        Selenide.refresh(); //Браузерная кнопка обновления страницы.

        Selenide.clearBrowserCookies();
        Selenide.clearBrowserLocalStorage();
        executeJavaScript("sessionStorage.clear();");
        //После исполнения этих команд, требуется обновить страницу.
    ...
    void selectors_examples() {
        ...
        $("").parent(); //Родитель
        $("").sibling(1); //Братья вниз
        $("").preceding(1); //Братья вверх
        $("").closest("div");
        $("").ancestor("div"); // the same as closest
        $("div:last-child"); //Псевдо- селектор. Это один из примеров. Выбирает последнего ребенка у элемента
    ...
    void actions_examples() {
        ...
        $("").setValue("text"); //Установить значение с заменой существующего
        $("").append("text"); //Добавляет к существующему значению
        $("").clear();
        $("").setValue(""); // clear
    ...
    void assertions_examples() {
        ...
        //longer timeouts
        $("").shouldBe(visible, Duration.ofSeconds(30)); //Настройка  таймАута для конкретной проверки
    ...
    void conditions_examples() {
        ...
        $("").should(matchText("[0-9]abc$")); //regex сложная проверка текста, рекомендуется оставлять комментарий, что этот regex делает

        $("").shouldHave(cssClass("red")); //проверяет наличие хотя бы одного класса.
        $("").shouldHave(cssClass("red"), cssClass("m15")); //Синтаксис проверки > 1 класса
        $("").shouldHave(cssValue("font-size", "12")); //Elements.Computed (справа)

        $("").shouldHave(value("25")); //Значение поля (текст)
    ...
    void collections_examples() {
        ...
        $$("div"); // does nothing! Не ищет без действия или проверки.
        ...
        $$("").shouldHave(texts("Alfa", "Beta", "Gamma")); //Нужно точное количество текстов, но по вхождению внутри каждого
        ...
        $$("").shouldHave(sizeGreaterThan(0)); //Количество элементов в коллекции
    ...
    void file_operation_examples() throws FileNotFoundException {
        ...
        //Alexey Vinogradov YouTube channel for more download options

        File file1 = $("a.fileLink").download(); // only for <a href=".."> links. Только для простых линков
